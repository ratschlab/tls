...
from tls.common.constants import STEPS_PER_HOUR, BINARY_TSH_URINE
...
def get_mid_resp_label(pre_resp_arr):
    """Transforms multiple level of severity annotation of respiratory failure events to a unique binary label"""

    ann_resp_arr = np.zeros_like(pre_resp_arr)
    for jdx in range(ann_resp_arr.size):
        if pre_resp_arr[jdx] in ["event_2", "event_3"]:
            ann_resp_arr[jdx] = 1
        elif pre_resp_arr[jdx] in ["UNKNOWN"]:
            ann_resp_arr[jdx] = np.nan
    return ann_resp_arr
...
def get_severe_resp_label(pre_resp_arr):
    """Transforms multiple level of severity annotation of respiratory failure events to a unique binary label"""

    ann_resp_arr = np.zeros_like(pre_resp_arr)
    for jdx in range(ann_resp_arr.size):
        if pre_resp_arr[jdx] in ["event_3"]:
            ann_resp_arr[jdx] = 1
        elif pre_resp_arr[jdx] in ["UNKNOWN"]:
            ann_resp_arr[jdx] = np.nan
    return ann_resp_arr
