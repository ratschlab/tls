...
from tls.data.preprocess import get_var_types
...
def extract_feat_patient(patient_sample, exclude, extract_presence=True):
    ...
    if extract_presence:
        ...
        all_feat_cols = [c for c in patient_sample.columns if c not in [constants.PID, constants.DATETIME]]
        presence_cols = ['presence_' + c for c in all_feat_cols]
        patient_feat[presence_cols] = patient_sample[all_feat_cols].notnull().astype(float)
        return patient_feat[
            [constants.PID, constants.DATETIME] + min_cols + max_cols + mean_cols + meas_cols + presence_cols]
